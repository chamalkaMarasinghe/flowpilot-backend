...
export async function login(input) {
    ...
    if (!user)
        throw new AppError(401, "No account found for that email");
...
export async function updateProfile(userId, input) {
    ...
    if (data.jobTitle !== undefined)
        user.jobTitle = data.jobTitle || undefined;
    if (data.department !== undefined)
        user.department = data.department || undefined;
...
export async function updatePreferences(userId, input) {
    ...
    if (data.sidebarOpen !== undefined)
        user.preferences.sidebarOpen = data.sidebarOpen;
    if (data.tableView !== undefined)
        user.preferences.tableView = data.tableView;
