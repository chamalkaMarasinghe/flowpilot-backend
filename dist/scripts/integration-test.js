...
async function run() {
    ...
    try {
        ...
        const me = await request(baseUrl, "/api/auth/me", { headers: authHeaders });
        ...
        const profilePatch = await request(baseUrl, "/api/auth/me", {
            method: "PATCH",
            headers: authHeaders,
        ...
        const meAfterPatch = await request(baseUrl, "/api/auth/me", { headers: authHeaders });
        ...
        const prefsPatch = await request(baseUrl, "/api/auth/me/preferences", {
            method: "PATCH",
            headers: authHeaders,
            body: JSON.stringify({ theme: "dark", tableView: "card", sidebarOpen: false }),
        });
        if (prefsPatch.status !== 200 ||
        ...
        const meWithPrefs = await request(baseUrl, "/api/auth/me", {
            headers: authHeaders,
        });
        ...
        const tasks = await request(baseUrl, "/api/tasks", { headers: authHeaders });
        ...
        const users = await request(baseUrl, "/api/users", { headers: authHeaders });
        ...
        const dashboard = await request(baseUrl, "/api/dashboard?period=month&taskFocus=today", {
            headers: authHeaders,
        });
        ...
        const filteredTasks = await request(baseUrl, "/api/tasks?status=OPEN&sort=priority", {
            headers: authHeaders,
        });
        ...
        const searchByTitle = await request(baseUrl, "/api/search/tasks?q=Test", {
            headers: authHeaders,
        });
        if (searchByTitle.status !== 200 || !searchByTitle.body.success || searchByTitle.body.data.length === 0) {
            throw new Error(`Search by title failed: ${JSON.stringify(searchByTitle.body)}`);
        }
        const searchByDesc = await request(baseUrl, "/api/search/tasks?q=Integration", {
            headers: authHeaders,
        });
        const descHits = searchByDesc.body.data ?? [];
        if (searchByDesc.status !== 200 ||
        ...
        const listSearch = await request(baseUrl, "/api/tasks?search=Test", { headers: authHeaders });
        if (listSearch.status !== 200 || !listSearch.body.success || listSearch.body.data.length === 0) {
        ...
        const userList = await request(baseUrl, "/api/users", { headers: userHeaders });
        const assigneeId = userList.body.data[0]?.id;
        if (!assigneeId)
            throw new Error("No users for assignee");
        ...
        const patched = await request(baseUrl, `/api/tasks/${taskId}`, {
            method: "PATCH",
            headers: userHeaders,
        ...
        const deleted = await request(baseUrl, `/api/tasks/${taskId}`, {
            method: "DELETE",
            headers: userHeaders,
        });
        ...
        const scopedTasks = await request(baseUrl, "/api/tasks", {
            headers: userHeaders,
        });
        if (scopedTasks.status !== 200 || !scopedTasks.body.success) {
            throw new Error(`Scoped tasks failed: ${JSON.stringify(scopedTasks.body)}`);
        }
        const selfDelete = await request(baseUrl, `/api/users/${me.body.data.user.id}`, {
            method: "DELETE",
            headers: authHeaders,
        });
        ...
        const deleteUserRes = await request(baseUrl, `/api/users/${creatorId}`, {
            method: "DELETE",
            headers: authHeaders,
        });
        ...
        console.log(`- Dashboard OK, ${dashboard.body.data.summary.totalTasks} task(s) in summary`);
        console.log(`- Filtered tasks OK, ${filteredTasks.body.data.length} OPEN task(s)`);
